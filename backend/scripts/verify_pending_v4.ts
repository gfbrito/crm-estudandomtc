...
async function verify() {
    console.log('Verifying pending webhooks for UnknownTest4...');
    const { data: pending, error: pendingError } = await supabaseAdmin
        .from('pending_webhooks')
        .select('*')
        .eq('origin', 'UnknownTest4');
    ...
    console.log('Verifying notifications for UnknownTest4...');
    ...
    if (notifError) {
        ...
        const unknownNotifs = notifs?.filter((n: any) => n.data?.origin === 'UnknownTest4');
